...
expanded_roster_points_per_game <- fantasy_teams_and_standings[
...
season_id_for_preseason <- case_when(
  game_type == "preseason" ~ season_id,
  game_type == "regular" ~ season_id - 1,
  game_type == "playoffs" ~ season_id - 2,
  TRUE ~ season_id
)

schedules_preseason <- get_schedule_info(
  season_id_for_preseason,
  season_schedules_by_id,
  team_codes,
  team_logo_urls,
  season_dates_and_types,
  ...
  season_dates_and_types |>
    filter(
      season_id == season_id_for_preseason
    ) |>
    pull(end_date)
)

schedule_preseason <- schedules_preseason["schedule"][[1]] |>
  mutate(
    game_date = game_date |> as.Date()
  )

schedule_to_date_preseason <- schedules_preseason["schedule_to_date"][[1]]
next_game_day_date_preseason <- schedules_preseason["next_game_day_date"][[1]]
todays_games_preseason <- schedules_preseason["todays_games"][[1]]
next_game_day_preseason <- schedules_preseason["next_game_day"][[1]]
...
standings_preseason <- compute_standings_changes_indicator(
  all_teams,
  schedule_to_date_preseason,
  current_date,
  schedule_preseason,
...
## Row
```{r, "announcement"}
if (Sys.Date() >= as.Date("2026-04-23")) {
  value_box(
    title = "Stay tuned!",
    showcase = bs_icon(
      "trophy",
    ...
    value = tags$p(
      "The site is frozen!! Finale results presented at awards ceremony at Alex and Noah's during playoffs.",
      style = "font-size: 4vh;"
    ),
    theme = "danger"
  )
...
trades_df <- lapply(
  names(team_rosters),
  generate_roster_trade_gt_table_row,
