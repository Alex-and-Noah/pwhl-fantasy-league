...
pwhl_player_box <- function(game_id) {
  tryCatch(
    expr = {
      URL <- glue::glue(
        "https://lscluster.hockeytech.com/feed/index.php?feed=statviewfeed&view=gameSummary&game_id={game_id}&key=694cfeed58c932ee&site_id=2&client_code=pwhl&lang=en&league_id=&callback=angular.callbacks._6"
      )
      ...
      suppressWarnings(
        expr = {
          home_goalie <- data.frame(
            row.names = NULL,
            (do.call(rbind, lapply(r$homeTeam$goalies, function(x) unlist(x))))
          ) %>%
            janitor::clean_names()
          away_goalie <- data.frame(
            row.names = NULL,
            (do.call(
              rbind,
              lapply(r$visitingTeam$goalies, function(x) unlist(x))
            ))
          ) %>%
            janitor::clean_names()

          home_skaters <- data.frame(
            row.names = NULL,
            (do.call(rbind, lapply(r$homeTeam$skaters, function(x) unlist(x))))
          ) %>%
            janitor::clean_names()
          away_skaters <- data.frame(
            row.names = NULL,
            (do.call(
              rbind,
              lapply(r$visitingTeam$skaters, function(x) unlist(x))
            ))
          ) %>%
            janitor::clean_names()
        }
      ...
      suppressWarnings(
        expr = {
          skaters <- skaters %>%
            ...
            dplyr::mutate(
              time_on_ice = round(
                (as.numeric(minute) * 60 + as.numeric(second)) / 60,
                1
              )
            ) %>%
            dplyr::select(c(
